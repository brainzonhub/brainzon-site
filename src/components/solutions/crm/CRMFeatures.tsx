...
import { crmFunctionFeatures } from "@/config/crm";
      ...
      <Container>
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="text-heading-2 font-bold text-foreground">
            CRM Solutions by Business Function
          </h2>
          <p className="mt-5 text-base leading-8 text-muted-foreground sm:text-lg">
            Empower every client-facing team with specialized pipelines, contact segments, and reporting modules to streamline operations.
          </p>
        </div>

        <div className="mt-12 grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {crmFunctionFeatures.map((solution) => {
            return (
              <article
                key={solution.title}
              ...
              >
                ...
                <div>
                  <span className="mb-6 flex size-12 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary shadow-inner transition-all duration-300 group-hover:border-primary/30 group-hover:bg-primary/20">
                    <CRMIcon name={solution.icon} className="size-6 text-primary" />
                  </span>
