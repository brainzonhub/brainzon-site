...
import { crmFinalCta } from "@/config/crm";

export function CRMCTA() {
  return (
    <section className="relative py-20 sm:py-28 bg-background transition-colors duration-300">
      <Container className="relative z-10">
        <div className="mx-auto max-w-4xl rounded-3xl border border-border/60 bg-card/40 backdrop-blur-md p-8 text-center shadow-card transition-all duration-500 hover:border-primary/50 hover:shadow-glow-primary sm:p-14 relative overflow-hidden group">
          ...
          {/* Subtle Glow Ring */}
          <div className="absolute -inset-10 -z-10 rounded-full bg-primary/5 blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
          ...
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 border border-primary/20 text-primary dark:text-cyan-300 dark:bg-cyan-300/10 dark:border-cyan-300/20 mb-7 transition-all duration-300 group-hover:scale-110">
            <Sparkles className="size-7 drop-shadow-[0_0_10px_rgba(34,211,238,0.3)]" />
          </div>

          {/* Heading */}
          <h2 className="text-3xl font-extrabold tracking-tight text-foreground sm:text-5xl">
            {crmFinalCta.headline}
          </h2>

          {/* Description */}
          <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-muted-foreground sm:text-lg">
            {crmFinalCta.description}
          </p>
